ALTER TABLE "oauth_providers" ADD COLUMN "builtin" boolean DEFAULT false NOT NULL;
...
SELECT gen_random_uuid()::text, 'github', 'GitHub', 'github', 'https://github.com/login/oauth/authorize', 'https://github.com/login/oauth/access_token', 'https://api.github.com/user', 'read:user user:email', ci.value, cs.value, true, true, now(), now()
...
--> statement-breakpoint
INSERT INTO "oauth_providers" ("id","key","label","icon","authorization_endpoint","token_endpoint","userinfo_endpoint","scopes","client_id","client_secret","enabled","builtin","created_at","updated_at")
SELECT gen_random_uuid()::text, 'discord', 'Discord', 'discord', 'https://discord.com/oauth2/authorize', 'https://discord.com/api/oauth2/token', 'https://discord.com/api/users/@me', 'identify email', ci.value, cs.value, true, true, now(), now()
...
--> statement-breakpoint
INSERT INTO "oauth_providers" ("id","key","label","icon","authorization_endpoint","token_endpoint","userinfo_endpoint","scopes","client_id","client_secret","enabled","builtin","created_at","updated_at")
SELECT gen_random_uuid()::text, 'modrinth', 'Modrinth', 'modrinth', 'https://modrinth.com/auth/authorize', 'https://api.modrinth.com/_internal/oauth/token', 'https://api.modrinth.com/v2/user', 'USER_READ USER_READ_EMAIL', ci.value, cs.value, true, true, now(), now()
